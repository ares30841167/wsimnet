...
class ContrastiveLoss(torch.nn.Module):
    # Similar pair: label set to 1
    # dissimilar pair: label set to 0
    ...
    def forward(self, distances, labels):
        return torch.mean(labels * torch.pow(distances, 2) +
                          (1 - labels) * torch.pow(torch.clamp(self.margin - distances, min=0.0), 2))


# Pairwise Loss From Graph Embedding Network
class PairwiseLoss(torch.nn.Module):
    # Similar pair: label set to 1
    # dissimilar pair: label set to -1
    ...
    def forward(self, distances, labels):
        return torch.relu(self.margin - labels * (1 - distances))
